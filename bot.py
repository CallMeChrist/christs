...
async def help():
	...
	embed.add_field(name='Other',value='Show other command list `-other`',inline=False)

	await bot.say(embed=embed)
	
@bot.command()
async def other():
	embed=discord.Embed(title='Other',description='Other command list :')
	embed.add_field(name='-nuke',value='Nuke `-nuke <name>`',inline=False)
	embed.add_field(name='-dm',value='DM a user what you say',inline=False)
	embed.add_field(name='-servercount',value='Show bot current server',inline=False)
	embed.add_field(name='-servericon',value='Show server icon',inline=False)
	embed.add_field(name='-serverid',value='Show server ID',inline=False)
...
async def serverid(ctx):
	  """Guild ID"""
	  await bot.say("***Server ID :*** `{}`".format(ctx.message.server.id))

bot.run('NTY0OTg2OTY0OTE5NDUxNjQ5.XKv4ZA.g5I-BI0pOHGvRLPwiZzcDYV5PHE')
